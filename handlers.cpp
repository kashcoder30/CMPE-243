...
TemperatureSensor temp1 = TemperatureSensor::getInstance();

CMD_HANDLER_FUNC(getTempHandler)
{
    /* cmdParams is a str passed to you after user's command.
     * If command was "get temperature" then cmdParams will be "temperature".
     *
     * Output is a CharDev class where the command came from, so
     * we can use this to output a temperature.
     */
    const char degSym = 0xB0; // Hex value for degree
    if(cmdParams == "temperature") {
        output.printf("Temperature is %f%cC\nThank you!\n", temp1.getCelsius(), degSym);
    }
    else {
        output.printf("ERROR for get temperature command\n");
    }
